...
export default function ShoppingListDetailPage() {
  ...
  const loadItems = (targetListId: number) => {
    const db = getDatabase();
    const result = db.exec(`
      SELECT li.id, li.list_id, li.product_id, li.name, li.quantity_value, li.quantity_label, li.estimated_unit_cost, li.notes, li.is_completed, li.created_at,
             p.name, p.buy_price, p.sell_price, p.quantity
    ...
    if (result[0]) {
      const loadedItems: ShoppingListItemWithProduct[] = result[0].values.map(row => ({
        ...
        notes: row[7] ? (row[7] as string) : null,
        is_completed: row[8] as number,
        created_at: row[9] as string,
        product_name: row[10] ? (row[10] as string) : null,
        product_buy_price: row[11] === null ? null : Number(row[11]),
        product_sell_price: row[12] === null ? null : Number(row[12]),
        product_quantity: row[13] === null ? null : Number(row[13])
      }));
      ...
      if (editingItemId) {
        db.run(
          `UPDATE shopping_list_items
           SET product_id = ?, name = ?, quantity_value = ?, quantity_label = ?, estimated_unit_cost = ?, notes = ?
           WHERE id = ?`,
          [
            productId,
            itemForm.name.trim(),
